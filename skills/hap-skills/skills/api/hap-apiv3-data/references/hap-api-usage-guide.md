...
鉴权凭证统一放在请求 **Header** 中，每个请求都必须携带。V3 支持三种鉴权方式：

| 方式 | Header 参数 | 创建者 | 操作身份 | 有效期 | 适用场景 |
| --- | --- | --- | --- | --- | --- |
| **AppKey + Sign** | `HAP-Appkey`、`HAP-Sign` | 管理员 | 应用管理员 | 长期 | 服务端集成 |
| **PAT** | `Authorization: Bearer {access_token}`、`HAP-Appid`（部分接口必填） | 个人 | 个人 | 可设定 | 个人脚本 / 工具 |
| **OAuth 2.0** | `Authorization: Bearer {access_token}`、`HAP-Appid`（部分接口必填） | 集成开发者 | 被授权用户 | 短期，自动刷新 | 第三方应用集成 |

- **AppKey + Sign**：应用密钥鉴权，由管理员在「应用 → 设置 → API 密钥」创建，以应用管理员身份访问数据。需要获取 **AppKey**、**Sign**、**应用 ID (app_id)**。
- **PAT**：个人访问凭证（Personal Access Token），自行创建，以个人身份操作，可设定有效期和权限范围。
- **OAuth 2.0**：用户通过 OAuth 集成完成授权，短期有效、支持自动刷新。
...
- 部门(Department): `in` / `eq` / `notin` 等（详见 2.1.1 对照表；V3 API 无 belongsto）
...
### 2.1.1 完整字段类型 / Code / 筛选运算符对照表（API 权威）

> ⭐ 本表是字段 **Code** 与 **筛选运算符** 的权威依据。注意：**V3 API 不支持 `belongsto`**——部门(Department)/关联(Relation) 等字段一律使用本表列出的运算符（如 `in`、`eq`、`notin`）。`-` 表示该字段不支持对应能力。
...
**⚠️ 关键点**:
1. 关联字段用 `in` 或 `eq` 操作符
2. V3 API 不支持 belongsto；部门(Department)字段用 in / eq / notin 等（见 2.1.1 对照表）
3. value 传入关联记录的 rowid 数组
...
| isempty | 所有 | 无 | 为空 |
| in / eq | Relation | ["rowid"...] | 关联（值为 rowid） |
| in / eq / notin | 部门(Department) | ["部门ID"...] | 部门筛选（V3 API 无 belongsto） |
